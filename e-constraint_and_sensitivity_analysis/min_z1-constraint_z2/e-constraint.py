...
"""model.pRr = pyo.Param(model.I, initialize = WorkforceData.resign_rate, doc = 'yearly resign rate for each type of workforce')"""
...
"""model.vXr = pyo.Var(model.I, model.J, bounds=(0.0,None), doc='yearly resignation workforce i type workers in year j', within=pyo.NonNegativeReals)"""
...
"""def eResignRate(model):
    for i in model.I:
        for j in model.J:
            if j == 0:
                return model.vXr[i,j] == (model.pB[i] * model.pRr[i])
            else:
                return model.vXr[i,j] == (model.vW[i,j-1] * model.pRr[i])
        
model.eResignRate = pyo.Constraint(rule = eResignRate, doc = 'yearly resignated i type workforce workforce')"""
...
model.eIdleness = pyo.Objective(rule = eIdleness, sense = pyo.minimize, doc='total idleness')"""
...
Solver.options['ranges'] = r"C:\Users\Emir\Desktop\case2\SA_Report_e-constraint.txt"
SolverResults = Solver.solve(model, tee=True)
model.eTotal_Cost.display()
model.vIdle.display()

pyo_SA.reorganize_SA_report(file_path_SA = r"C:\Users\Emir\Desktop\case2\SA_Report_e-constraint.txt",\
                            file_path_LP_labels = r"C:\Users\Emir\Desktop\case2\model_labels_e-constraint.lp",\
                                file_path_LP_nolabels = r"C:\Users\Emir\Desktop\case2\model_nolabels_e-constraint.lp")
