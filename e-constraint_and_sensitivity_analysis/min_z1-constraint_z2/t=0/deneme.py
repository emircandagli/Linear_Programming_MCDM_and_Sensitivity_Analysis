# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 20:13:43 2024
...
e = 3051 + ((3171-3051) * 0 / 4)

def eConstraint(model):
    return model.vIdle <= e

model.eConstraint = pyo.Constraint(rule = eConstraint)  
...
model.eTotal_Cost = pyo.Objective(rule = eTotal_Cost,sense = pyo.minimize)

#Export the open form of the model with the user defined labels
model.write('model_labels_e-constraint.lp', io_options={'symbolic_solver_labels': True})
#Export the open form of the model without the user defined labels
model.write('model_nolabels_e-constraint.lp', io_options={'symbolic_solver_labels': False})
...
Solver.options['ranges'] = r"C:\Users\Emir\Desktop\case2\e-constraint_and_sensitivty_analysis\min_z1-constraint_z2\t=0\SA_Report_e-constraint.txt"

model.eTotal_Cost.display()
model.vIdle.display()


pyo_SA.reorganize_SA_report(file_path_SA = r"C:\Users\Emir\Desktop\case2\e-constraint_and_sensitivty_analysis\min_z1-constraint_z2\t=0\SA_Report_e-constraint.txt",\
                            file_path_LP_labels = r"C:\Users\Emir\Desktop\case2\e-constraint_and_sensitivty_analysis\min_z1-constraint_z2\t=0\model_labels_e-constraint.lp",\
                                file_path_LP_nolabels = r"C:\Users\Emir\Desktop\case2\e-constraint_and_sensitivty_analysis\min_z1-constraint_z2\t=0\model_nolabels_e-constraint.lp")
        
reduced_cost_dict={str(key):model.rc[key] for key in model.rc.keys()} 
Reduced_Costs_print =pd.DataFrame.from_dict(reduced_cost_dict,orient="index", columns=["reduced cost"]) 
Reduced_Costs_print.to_excel(r'C:\Users\Emir\Desktop\case2\e-constraint_and_sensitivty_analysis\min_z1-constraint_z2\t=0\ReducedCosts.xlsx',sheet_name='ReducedCosts') 
...
u_slack_dict = {
     ...
     **{str(con):con.uslack() for con in model.component_objects(pyo.Constraint) if not con.is_indexed()},
...
#     # get all the indices then retrieve the slacks for each index of constraint
     **{k:v for con in model.component_objects(pyo.Constraint) if con.is_indexed() for k,v in {'{}[{}]'.format(str(con),key):con[key].uslack()
     for key in con.keys()}.items()}
 }
 
l_slack_dict = {
     # lslacks for non-indexed constraints
     **{str(con):con.lslack() for con in model.component_objects(pyo.Constraint) if not con.is_indexed()},
     # indexed constraint lslack
     # loop through the indexed constraints
     # get all the indices then retrieve the slacks for each index of constraint
     **{k:v for con in model.component_objects(pyo.Constraint) if con.is_indexed() for k,v in {'{}[{}]'.format(str(con),key):con[key].lslack()
     for key in con.keys()}.items()}
 }
...
Shadow_Prices_print.to_excel(r'C:\Users\Emir\Desktop\case2\e-constraint_and_sensitivty_analysis\min_z1-constraint_z2\t=0\ShadowPrices.xlsx',  sheet_name='ShadowPrices')