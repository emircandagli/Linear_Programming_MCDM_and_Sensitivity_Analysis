...
def read_LP_file(file_path_LP):
    ...
    df_LP = pd.read_fwf(file_path_LP, skiprows=0, header=0)
    ...
    df_LP_variables = df_LP_variables.str.split('<=')
    
    # Expand the list of split values into separate columns and store the second column in a list
    LP_variable_names = df_LP_variables.apply(pd.Series).iloc[:,1].to_list()
